...
const CompactTabInput = memo(({ label, value, onChange, onSync, sheetId, isSyncing }: {
  label: string;
  value: string;
  ...
  return (
    <div className="flex flex-col gap-1.5 p-3 rounded-lg bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-700/50 hover:border-blue-400/30 transition-all group">
      <div className="flex justify-between items-center">
        ...
        {isSyncing ? <Loader2 size={10} className="animate-spin text-blue-500" /> :
         status === 'checking' ? <Loader2 size={10} className="animate-spin text-slate-400" /> :
...
const AccountCard = ({ userProfile, sheetId, sheetUrl, isAuthLoading, onSignIn, onSignOut, onPicker }: {
  userProfile: UserProfile | null;
  sheetId: string;
  sheetUrl: string;
                ...
                <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-700/50">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="flex-1 relative group">
                             <input 
                                type="text" 
                                value={sheetId ? `Sheet ID: ${sheetId}` : ''} 
                                readOnly 
                        ...
                        >
                            <Search size={18} />
                            {sheetId ? 'Change Sheet' : 'Select Sheet'}
                        </button>
                    </div>
                    {sheetId && (
...
const PreferencesCard = ({ isDarkMode, toggleTheme, onWipe, onViewPrivacy, onViewTerms }: {
  isDarkMode: boolean;
  toggleTheme: () => void;
  onWipe: () => void;
  onViewPrivacy: () => void;
  ...
  <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-6 rounded-2xl shadow-sm flex flex-col gap-6 h-full">
      ...
      <div className="mt-auto pt-6 border-t border-slate-100 dark:border-slate-700">
          ...
          <div className="space-y-2">
              ...
              >
                  <span className="flex items-center gap-2"><ShieldCheck size={12} /> Privacy Policy</span> <ArrowRight size={10} />
              </button>
              <button 
                  onClick={onViewTerms}
                  className="w-full flex items-center justify-between px-3 py-2 text-[10px] font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 border border-amber-100 dark:border-amber-500/20 rounded-lg hover:bg-amber-100 dark:hover:bg-amber-500/20 transition-all"
              >
                  <span className="flex items-center gap-2"><Scale size={12} /> Terms of Service</span> <ArrowRight size={10} />
              </button>
              <button 
                  onClick={onWipe}
                  className="w-full flex items-center justify-center gap-2 text-[10px] font-bold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-500/10 border border-red-100 dark:border-red-500/20 px-4 py-2.5 rounded-xl hover:bg-red-100 dark:hover:bg-red-500/20 transition-all mt-2"
              >
...
const MappingCard = ({ config, onConfigChange, onSync, isSyncing, syncingTabs, syncStatus }: {
  config: SheetConfig;
    ...
    <div className="max-w-5xl mx-auto space-y-8 animate-fade-in pb-10">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        <AccountCard 
            ...
            onSignOut={onSignOut}
            onPicker={handleOpenPicker}
        />
        <PreferencesCard 
            isDarkMode={isDarkMode}
            toggleTheme={toggleTheme}
            onWipe={handleWipeData}
            onViewPrivacy={() => onViewChange(ViewState.PRIVACY)}
            onViewTerms={() => onViewChange(ViewState.TERMS)}
        />
      </div>
      <MappingCard 
        config={config}
