...
import { BadgeDollarSign, Loader2, TrendingUp, TrendingDown, CreditCard, Activity, GitMerge, BarChart2, X, Percent } from 'lucide-react';
...
const ChartContainer: React.FC<ChartContainerProps> = ({ title, icon: Icon, iconColor, children, subTitle, controls }) => (
...
export const IncomeView: React.FC<IncomeViewProps> = ({ incomeData, expenseData, detailedExpenses, isLoading = false, isDarkMode = true }) => {
  ...
  const expenseStats = useMemo(() => {
    ...
    const chartData = expenseData.map(e => {
        ...
        if (expenseFilter === 'All') {
            relevantTotal = e.total; 
        } else {
            relevantTotal = e.categories[expenseFilter] || 0;
        }
        
        total += relevantTotal;
        if (relevantTotal > 0) count++;
        if (e.date.startsWith(String(currentYear)) && e.date <= todayISO) {
            ytd += relevantTotal;
        }
  ...
  const sankeyData = useMemo(() => {
      ...
      detailedExpenses.categories.forEach((cat, cIdx) => {
          // If we have a filter, only process the selected category
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-20">
      <header>
        ...
        <p className="text-slate-500 dark:text-slate-400">Cash flow analysis and detailed spending breakdown.</p>
      </header>
